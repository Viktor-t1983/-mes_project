...
def pause_operation(db: Session, operation_id: int, reason: str = None):
    """
    Поставить операцию на паузу.
    """
    db_operation = db.query(Operation).filter(Operation.id == operation_id).first()
    if db_operation and db_operation.status == "in_progress":
        db_operation.status = "paused"
        
        # Добавляем информацию о паузе
        if not db_operation.pause_events:
            db_operation.pause_events = {}
        
        from datetime import datetime
        pause_time = datetime.now().isoformat()
        db_operation.pause_events[pause_time] = {
            "reason": reason,
            "paused_by": "system"
        }
...
def resume_operation(db: Session, operation_id: int):
    """
    Возобновить операцию после паузы.
    """
    db_operation = db.query(Operation).filter(Operation.id == operation_id).first()
    if db_operation and db_operation.status == "paused":
        db_operation.status = "in_progress"
        db.commit()
        db.refresh(db_operation)
    return db_operation
