...
def test_fixed_system():
    ...
    test_cases = [
        ...
        ("/api/v1/operations", {"name": "Тестовая операция", "description": "Описание операции"}, "Операция"),
        ("/api/v1/defects", {"defect_type": "Качество", "defect_description": "Тестовое отклонение", "severity": "medium"}, "Отклонение"),
        ("/api/v1/projects", {"name": "Тестовый проект", "description": "Описание проекта"}, "Проект")
    ]
    ...
    # 3. Тестируем управление операциями если операция создана
    ...
    # 5. Тестируем QR коды (должны возвращать изображения)
    ...
    for endpoint, description in qr_endpoints:
        try:
            ...
            if response.status_code == 200 and response.headers.get('content-type', '').startswith('image/'):
                ...
                print(f"   ❌ {description}: {response.status_code} ({response.headers.get('content-type', 'N/A')})")
                results.append((description, "❌"))
        except Exception as e:
            print(f"   🚫 {description}: {e}")
            results.append((description, "🚫"))
